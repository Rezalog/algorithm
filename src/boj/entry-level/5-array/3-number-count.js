/*
1. 단계별로 풀어보기 - 1차원 배열 - 2577 - 숫자의 개수
...
for(let zeroToNine=0; zeroToNine<=9; zeroToNine++) {
    
    let count=0;

    for(let index=0; index<numberArr.length; index++) {
        // zeroToNine=0 : 0의 카운트 수 => 
        //                0과 같은경우를 numberArr를 모두 돌아 같은경우 count++
        //     :
        if(numberArr[index] === zeroToNine) {
            count++;
        }
    }
    console.log(count);
