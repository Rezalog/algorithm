...
function numberSearch(str) { 
    ...
    for(let i=0; i<str.length; i++) {
        // if(typeof Number(trimedStr[i])==='number') {
        //     strNum += Number(trimedStr[i]);
        // }
        if(digit.includes(str[i])) {
            strNum += Number(str[i]);
        } else if(str[i]!==' ') {
            trimedStr += str[i];
        }
    }
    return Math.round(strNum/trimedStr.length);
    // return strNum;
}
console.log(numberSearch('Hello6 '));
console.log(numberSearch('Hello6 9World 2,'));
